...
class ToolTracingModel(SQLModel, table=True):
    __tablename__ = "tracing_tool"  # type: ignore
...
class GenerationTracingModel(SQLModel, table=True):
    __tablename__ = "tracing_generation"  # type: ignore

    id: int | None = Field(default=None, primary_key=True)
    trace_id: str = ""
    span_id: str = ""
    type: Literal["chat.completions", "responses"] = Field(
        default="chat.completions", sa_column=Column(String)
    )

    input: Sequence[Mapping[str, Any]] | None = Field(
        default=None, sa_column=Column(JSON)
    )
    output: Sequence[Mapping[str, Any]] | None = Field(
        default=None, sa_column=Column(JSON)
    )
    model: str = Field(sa_column=Column(String))
    model_configs: Mapping[str, Any] | None = Field(
        default=None, sa_column=Column(JSON)
    )
