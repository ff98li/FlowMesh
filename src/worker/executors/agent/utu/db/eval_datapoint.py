...
class DatasetSample(SQLModel, table=True):
    ...
    index: int | None = Field(
        default=None
    )  # The index of the datapoint in the dataset, starting from 1
    source: str = ""  # dataset name for mixed dataset
    source_index: int | None = Field(
        default=None
    )  # The index of the datapoint in the source dataset, if available
...
class EvaluationSample(UTUBaseModel, SQLModel, table=True):
    ...
    trajectory: str | None = Field(
        default=None
    )  # deprecated, use trajectories instead for multi-agents
