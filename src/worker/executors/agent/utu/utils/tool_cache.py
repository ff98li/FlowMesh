    ...
    def decorator_file(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if cache_file.exists():
                ...
                if (
                    expire_time is None
                    or (time.time() - cache_data["metadata"]["timestamp"]) < expire_time
                ):
                    logger.debug(
                        f"🔄 Using cached result for {func_name} from {cache_file}"
                    )
    ...
    def decorator_db(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            with SQLModelUtils.create_session() as session:
                ...
                if if_exist and (
                    expire_time is None
                    or (time.time() - if_exist.timestamp) < expire_time
                ):
...
def async_file_cache(
    cache_dir: str | pathlib.Path = DIR_CACHE,
    expire_time: int | None = None,
    mode: Literal["db", "file"] = "db",
):
    """Decorator to cache async function results to local files.

    Args:
        cache_dir (str|pathlib.Path): Directory to store cache files
        expire_time (int | None): Cache expiration time in seconds, None means no
            expiration
    """
