...
class ChatCompletionConverter(Converter):
    ...
    def items_to_messages(
        ...
        # {'id': '__fake_id__', 'summary': [{'text': '...', 'type': 'summary_text'}],
        # 'type': 'reasoning'}
    ...
    @classmethod
    def filter_items(
        cls, items: str | Iterable[TResponseInputItem]
    ) -> str | list[TResponseInputItem]:
        if isinstance(items, str):
            return items
        filtered_items = []
        for item in items:
            if isinstance(item, dict) and item.get("type", None) == "reasoning":
                continue
    ...
    def items_to_dict(cls, items: str | Iterable[TResponseInputItem]) -> list[dict]:
        ...
        for item in items:
            if msg := Converter.maybe_easy_input_message(item):
                ...
                msg.update(
                    {"role": "tool", "content": msg["results"]}  # type: ignore[misc]
                )
                result.append(msg)
                ...
                msg.update(
                    {
                        "role": "assistant",
                        "content": f"{msg['name']}({msg['arguments']})",
                    }  # type: ignore[misc]
                )
                result.append(msg)
                ...
                msg.update(
                    {
                        "role": "assistant",
                        "content": msg["summary"],  # type: ignore
                ...
                result.append(
                    {"role": "assistant", "content": f"Unknown message type: {item}"}
                )
        return result
...
class AgentsUtils:
    ...
    def get_agents_model(
        type: Literal["responses", "chat.completions", "litellm"] | None = None,
        model: str | None = None,
        base_url: str | None = None,
        api_key: str | None = None,
    ) -> Model:
        type_str: str = type or os.getenv(  # type: ignore
            "UTU_LLM_TYPE",
            "chat.completions",
        )
        model_str: str = model or os.getenv(  # type: ignore
            "UTU_LLM_MODEL",
            "gpt-5-mini",
        )
        ...
        if type_str == "chat.completions":
            return OpenAIChatCompletionsModel(
                model=model_str, openai_client=openai_client
            )
        elif type_str == "responses":
            return OpenAIResponsesModel(model=model_str, openai_client=openai_client)
        elif type_str == "litellm":
            # Ref: https://docs.litellm.ai/docs/providers
            # NOTE: should set .evn properly! e.g. AZURE_API_KEY, AZURE_API_BASE,
            ...
            raise ValueError("Invalid type: " + type_str)

    @staticmethod
    def get_trajectory_from_agent_result(
        agent_result: RunResult | RunResultStreaming, agent_name: str | None = None
    ) -> dict:
        if agent_name is None:
            agent_name = agent_result.last_agent.name
        return {
            "agent": agent_name,
            "trajectory": ChatCompletionConverter.items_to_messages(
                agent_result.to_input_list()
            ),
        }
    ...
    def print_new_items(new_items: list[RunItem]) -> None:
        ...
        for new_item in new_items:
            ...
            if isinstance(new_item, MessageOutputItem):
                ...
                assert isinstance(
                    new_item.raw_item, ResponseFunctionToolCall
                )  # DO NOT use openai's built-in tools
                PrintUtils.print_info(
                    f"{agent_name}: Calling a tool: {new_item.raw_item.name}"
                    f"({json.loads(new_item.raw_item.arguments)})"
                )
        ...
        async for event in result:
            ...
            if isinstance(event, RawResponsesStreamEvent):
                ...
                if event.data.type == "response.output_text.delta":
                    ...
                    PrintUtils.print_info(
                        f"Unknown event type: {event.data.type}! {event}"
                    )
                    # raise ValueError(f"Unknown event type: {event.data.type}")
            elif isinstance(event, RunItemStreamEvent):
                item: RunItem = event.item
                if item.type == "message_output_item":
                    pass  # already handled `response.output_text.delta`
                    # PrintUtils.print_bot(
                    #     f"<{item.agent.name}> "
                    #     f"{ItemHelpers.text_message_output(item).strip()}"
                    # )
                elif item.type == "handoff_call_item":  # same as `ToolCallItem`
                    PrintUtils.print_bot(
                        f"[handoff_call] {item.raw_item.name}"
                        f"({item.raw_item.arguments})"
                    )
                elif item.type == "handoff_output_item":
                    PrintUtils.print_info(
                        f">> Handoff from {item.source_agent.name} to "
                        f"{item.target_agent.name}"
                    )
                elif item.type == "tool_call_item":
                    raw_item = cast(ResponseFunctionToolCall, item.raw_item)
                    PrintUtils.print_bot(
                        f"<{item.agent.name}> [tool_call] {raw_item.name}"
                        f"({raw_item.arguments})"
                    )
                elif item.type == "tool_call_output_item":
                    PrintUtils.print_tool(
                        f"<{item.agent.name}> [tool_output] {item.output}"
                    )  # item.raw_item
                    ...
                    PrintUtils.print_info(
                        f"  >>> Skipping item: {item.__class__.__name__}"
                    )
    ...
    def convert_sp_input(
        ...
        if messages[0].get("role", None) == "system":
            return (
                ...
                list(messages[1:]),  # type: ignore
            )
        return None, messages  # type: ignore
    ...
    def convert_tool(tool: ChatCompletionToolParam) -> FunctionTool:
        ...
        return FunctionTool(
            ...
            params_json_schema=tool["function"].get("parameters") or {},
            on_invoke_tool=None,  # type: ignore
        )
...
class SimplifiedOpenAIChatCompletionsModel(OpenAIChatCompletionsModel):
    ...
    async def query_one(self, **kwargs) -> list[ChatCompletionMessageParam]:
        system_instructions, input = AgentsUtils.convert_sp_input(kwargs["messages"])
        model_settings = AgentsUtils.convert_model_settings(kwargs)  # type: ignore
        tools: list[Tool] = [
            AgentsUtils.convert_tool(tool) for tool in kwargs.get("tools", [])
        ]
