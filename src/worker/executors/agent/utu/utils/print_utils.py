...
init(
    autoreset=True
)  # Reset color to default (autoreset=True handles this automatically)

COLOR_DICT: dict[str, str] = defaultdict(lambda: Style.RESET_ALL)
COLOR_DICT.update(
    {
...
class PrintUtils:
    ...
    def print_input(prompt_text: str):
        ...
        # user_input = input(
        #     COLOR_DICT[prompt_color] + prompt_text + COLOR_DICT[input_color]
        # )
    ...
    def print_bot(
        ...
        PrintUtils.print_info(
            msg, color=color, add_prefix=add_prefix, prefix=prefix, end=end, flush=flush
        )
