...
class SimplifiedAsyncOpenAI(AsyncOpenAI):
    """Simplified OpenAI client for chat.completions and responses API, with default
    config"""

    def __init__(
        self,
        *,
        type: Literal["chat.completions", "responses"] | None = None,
        ...
        **kwargs: dict,
        ...
        self.type_create_params = (
            ...
            else OpenAIResponsesParamsKeys
        )
    ...
    def _process_kwargs(self, kwargs: dict) -> dict:
        ...
        for k, v in kwargs.items():
            if k in self.type_create_params:
                default_config[k] = v
        default_config["model"] = default_config.get(
    ...
    async def query_one(self, **kwargs) -> str:
        ...
        WARNING: Only for basic text i/o usage! You should not use the method with
        querying with customized configs!
        """
        if "stream" in kwargs:
            assert kwargs["stream"] is False, "stream is not supported in `query_one`"

        if self.type == "chat.completions":
            chat_completion: ChatCompletion = await self.chat_completions_create(
                **kwargs
            )  # type: ignore
            return chat_completion.choices[0].message.content or ""
        elif self.type == "responses":
            response: Response = await self.responses_create(**kwargs)  # type: ignore
    ...
    async def chat_completions_create(
        ...
        assert (
            self.type == "chat.completions"
        ), "`chat_completions_create` is not supported for responses API"
        ...
        params = self.process_chat_completion_params(kwargs, self.default_config)
        return await self.chat.completions.create(**params)  # type: ignore
    ...
    async def responses_create(
        ...
        assert (
            self.type == "responses"
        ), "`responses_create` is not supported for chat.completions API"
        params = self.process_responses_params(kwargs, self.default_config)
        return await self.responses.create(**params)  # type: ignore

    def process_chat_completion_params(
        self, kwargs: dict, default_config: dict
    ) -> OpenAIChatCompletionParams:
        """Process chat completion params, convert str to list of messages, merge
        default config"""
        assert "messages" in kwargs
        if isinstance(kwargs["messages"], str):
            kwargs["messages"] = [{"role": "user", "content": kwargs["messages"]}]
        return self._merge_default_config(kwargs, default_config)  # type: ignore

    def process_responses_params(
        self, kwargs: dict, default_config: dict
    ) -> OpenAIResponsesParams:
        """Process responses params, convert str to list of messages, merge default
        config"""
    ...
    def check_known_keys(self, kwargs: dict, known_keys: Iterable[str]) -> set:
        """Check if all keys in kwargs are in known_keys"""
        unknown_keys = set(kwargs.keys()).difference(known_keys)
        return unknown_keys
