...
def setup_logging(level: Literal["WARNING", "INFO", "DEBUG"] = "WARNING") -> None:
    ...
    color_formatter = ColoredFormatter(
        ...
        "%(log_color)s%(levelname)s%(reset)s - %(filename)s:%(lineno)d - "
...
def get_logger(
    name: str,
    level: int | Literal["INFO", "WARNING", "ERROR", "CRITICAL"] | None = None,
) -> logging.Logger:
    ...
    logger.error_exc = log_error_with_exc  # type: ignore[attr-defined]
    return logger
