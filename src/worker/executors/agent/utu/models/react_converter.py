...
class ReactConverter:
    ...
    def preprocess(self, input: ConverterPreprocessInput) -> ConverterPreprocessInput:
        ...
        """
        converted_sp = self._handle_sp(
            input.system_instructions, input.tools, input.handoffs
        )
        converted_input = self._handle_input(input.system_instructions, input.input)
        converted_model_settings = self._handle_model_settings(
            input.model_settings or ModelSettings()
        )
        return ConverterPreprocessInput(
            system_instructions=converted_sp,
            input=converted_input,
            model_settings=converted_model_settings,
        )
    ...
    def _handle_input(
        ...
        for item in input:
            ...
            if Converter.maybe_easy_input_message(
                item
            ) or Converter.maybe_input_message(item):
            ...
            # type == "function_call"
                ...
                message = EasyInputMessageParam(
                    ...
                    content=self.template_action.render(
                        ...
                        action_arguments=func_call["arguments"],
                    ),
                )
    ...
    def _parse_react_output(self, text_output: str) -> list[ResponseOutputItem]:
        ...
        if self.action_str in text_output:
            ...
            message = ChatCompletionMessage(
                ...
                tool_calls=[
                    ChatCompletionMessageFunctionToolCall(
                        function=function, id="FAKE_ID", type="function"
                    )
                ],
            )
