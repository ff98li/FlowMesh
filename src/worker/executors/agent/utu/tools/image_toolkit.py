    ...
    async def image_qa(self, image_path: str, question: str | None = None) -> str:
        ...
        if not question:
            messages = [
                ...
                {
                    "role": "user",
                    "content": [{"type": "image_url", "image_url": {"url": image_str}}],
                },
            ]
            ...
            output = (
                "You did not provide a particular question, so here is a detailed "
                f"caption for the image: {output}"
            )
        else:
            messages = [
