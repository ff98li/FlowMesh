...
def register_tool(name: str | Callable | None = None):
...
class AsyncBaseToolkit(abc.ABC):
    ...
    _tools_map: dict[str, Callable] | None = None
        ...
        if self._tools_map is None:
            ...
            for attr_name in dir(self):
                ...
                if callable(attr) and getattr(attr, "_is_tool", False):
                    self._tools_map[attr._tool_name] = attr  # type: ignore[attr-defined]
    ...
    async def get_tools_map_func(self) -> dict[str, Callable]:
        """Get tools map. It will filter tools by config.activated_tools if it is
        not None."""
        if self.config.activated_tools:
            assert all(
                tool_name in self.tools_map for tool_name in self.config.activated_tools
            ), (
                f"Error config activated tools: {self.config.activated_tools}! "
                f"available tools: {self.tools_map.keys()}"
            )
            tools_map = {
                tool_name: self.tools_map[tool_name]
    ...
    async def get_tools_in_agents(self) -> list[Tool]:
    ...
    async def get_tools_in_openai(self) -> list[ChatCompletionToolParam]:
    ...
    async def get_tools_in_mcp(self) -> list[types.Tool]:
        ...
        tools = await self.get_tools_in_agents()
        return [
            MCPConverter.function_tool_to_mcp(tool)
