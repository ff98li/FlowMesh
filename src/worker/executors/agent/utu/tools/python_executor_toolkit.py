...
def _execute_python_code_sync(code: str, workdir: str):
    ...
    try:
        ...
        if hasattr(shell, "history_manager"):
            shell.history_manager.enabled = False  # type: ignore
        ...
        try:
            shell.atexit_operations = lambda: None  # type: ignore[method-assign]
        ...
        return {
            ...
            "success": False,
            "message": (
                f"Code execution failed, error message:\n{str(e)},\n"
...
class PythonExecutorToolkit(AsyncBaseToolkit):
    ...
    async def execute_python_code(
        self, code: str, workdir: str | None = None, timeout: int = 30
    ) -> dict:
        """
        ...
        Args:
            ...
            workdir (str | None): The working directory for the execution.
                Defaults to "./run_workdir".
