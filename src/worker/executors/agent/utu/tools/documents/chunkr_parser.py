...
class ChunkrParser:
    def __init__(self, config: dict) -> None:
        ...
        from chunkr_ai.models import Configuration  # type: ignore[import-untyped]

        self.chunkr = Chunkr(api_key=config.get("CHUNKR_API_KEY"))
        self.chunkr.config = Configuration(  # type: ignore
    ...
    async def parse(self, path: str) -> str:
        ...
        """
        task = await self.chunkr.upload(path)  # type: ignore
