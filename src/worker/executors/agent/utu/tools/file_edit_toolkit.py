...
class FileEditToolkit(AsyncBaseToolkit):
    def __init__(self, config: ToolkitConfig | None = None) -> None:
        ...
        logger.info(
            f"FileEditToolkit initialized with output directory: {self.work_dir}, "
    ...
    @register_tool
    async def edit_file(
        self, file_name: str, diff: str
    ) -> None:  # TODO: return edit result!
        r"""Edit a file by applying the provided diff.

        Args:
            file_name (str): The name of the file to edit.
            diff (str): (required) One or more SEARCH/REPLACE blocks following this
                exact format:
                ```
                <<<<<<< SEARCH
        ...
        try:
            ...
            for search_text, replace_text in matches:
                if search_text in modified_content:
                    ...
                    logger.warning(
                        f"Search text not found in file: {search_text[:50]}..."
                    )
