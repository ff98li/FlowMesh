...
class WikipediaSearchTool(AsyncBaseToolkit):
    """
    WikipediaSearchTool searches Wikipedia and returns a summary or full text of the
    given topic.
    ...
    def __init__(self, config: ToolkitConfig | dict | None = None) -> None:
        ...
        try:
            ...
            raise ImportError(
                "You must install `wikipedia-api` to run this tool: for instance run "
    ...
    async def wikipedia_search(self, query: str) -> str:
        """Searches Wikipedia and returns a summary or full text of the given topic,
        along with the page URL.
    ...
    @register_tool
    async def search_wikipedia_revisions(
        self, entity: str, year: int, month: int
    ) -> str:
        """Get the revisions of a Wikipedia entity in a given month, return the
        revision url.
