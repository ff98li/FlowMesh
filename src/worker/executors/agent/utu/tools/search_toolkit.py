...
class SearchToolkit(AsyncBaseToolkit):
    ...
    NOTE:
        - Please configure the required env variables!
          See `configs/agents/tools/search.yaml`
    ...
    async def web_qa(self, url: str, query: str) -> str:
        """Ask question to a webpage, you will get the answer and related links from
        the specified url.
        ...
        query = query or (
            "Summarize the content of this webpage, in the same language as the "
            "webpage."
    ...
    async def _extract_links(self, url: str, content: str, query: str) -> str:
        template = TOOL_PROMPTS["search_related"].format(
            url=url, content=content, query=query
        )
        return await self.llm.query_one(
            messages=[{"role": "user", "content": template}],
            **self.config.config_llm.model_params.model_dump(),
