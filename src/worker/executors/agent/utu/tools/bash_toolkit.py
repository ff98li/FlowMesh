...
* When invoking this tool, the contents of the \"command\" parameter does NOT need to
  be XML-escaped.\n
* You don't have access to the internet via this tool.\n
* You do have access to a mirror of common linux and python packages via apt and
  pip.\n
* State is persistent across command calls and discussions with the user.\n
* To inspect a particular line range of a file, e.g. lines 10-25, try 'sed -n 10,25p
  /path/to/the/file'.\n
* Please avoid commands that may produce a very large amount of output.\n
* Please run long lived commands in the background, e.g. 'sleep 10 &' or start a server
  in the background."
"""
...
class BashToolkit(AsyncBaseToolkit):
    def __init__(self, config: ToolkitConfig | None = None) -> None:
        ...
        self.child, self.custom_prompt = self.start_persistent_shell(
    ...
    def start_persistent_shell(timeout: int):
        ...
        if sys.platform == "win32":
            child = pexpect.spawn(
                "cmd.exe", encoding="utf-8", echo=False, timeout=timeout
            )
            ...
            child = pexpect.spawn(
                "/bin/bash", encoding="utf-8", echo=False, timeout=timeout
            )
    ...
    def run_command(child, custom_prompt: str, cmd: str) -> str:
        ...
        # pexpect puts the matched prompt in child.after and everything before it in
        # child.before.
    ...
    async def run_bash(self, command: str) -> str:
        ...
        for banned_str in self.banned_command_strs:
            if banned_str in command:
                return (
                    "Command not executed due to banned string in command: "
                    f"{banned_str} found in {command}."
                )
