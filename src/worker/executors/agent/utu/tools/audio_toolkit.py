...
class AudioToolkit(AsyncBaseToolkit):
    def __init__(self, config: ToolkitConfig | None = None) -> None:
        ...
        self.audio_client = SimplifiedAsyncOpenAI(
            api_key=EnvUtils.get_env(
                "UTU_AUDIO_LLM_API_KEY"
            ),  # NOTE: you should set these envs in .env
        ...
        self.llm = SimplifiedAsyncOpenAI(
            **config.config_llm.model_provider.model_dump()
        )
    ...
    async def transcribe(self, md5: str) -> dict:
        ...
        transcript: TranscriptionVerbose = (
            await self.audio_client.audio.transcriptions.create(
                model=self.audio_model or "whisper-1",
                file=open(fn, "rb"),
    ...
    async def audio_qa(self, audio_path: str, question: str) -> str:
        ...
        """
        logger.debug(
            f"Processing audio file `{audio_path}` with question `{question}`."
        )
        ...
        messages = [
            {
                "role": "system",
                "content": "You are a helpful assistant specializing in audio "
                "analysis.",
            },
            {
                "role": "user",
