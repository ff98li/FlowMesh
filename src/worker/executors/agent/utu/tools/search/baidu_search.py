...
class BaiduSearch:
    ...
    def __init__(self, config: dict | None = None) -> None:
        ...
        self.content_filter = (
            ContentFilter(search_banned_sites) if search_banned_sites else None
        )
    ...
    async def search(self, query: str, num_results: int = 5) -> str:
        ...
        if self.content_filter:
            results = self.content_filter.filter_results(
                res["data"], num_results, key="url"
            )
        else:
    ...
    async def search_baidu(self, query: str) -> dict:
        ...
        - WARNING: Uses web scraping which may be subject to rate limiting or anti-bot
          measures.
