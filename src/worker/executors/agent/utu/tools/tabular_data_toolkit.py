...
TEMPLATE_COLUMN_QA = (
    "You are a data analysis agent that extracts and summarizes data structure "
    "information from tabular data files (CSV, Excel, etc.).\n\n"
    "<column_info>\n"
    "{column_info}\n"
    "</column_info>\n\n"
    "You should extract the file structure(e.g. the delimiter), and provide detail "
    "column information (e.g. column_name, type, column explanation and sample values) "
    "for each column.\n"
    "<output_format>\n"
    "### File Structure\n"
    ...
    "|-------------|------|-------------|--------------|\n"
    "| name_of_column1 | type_of_column1 | explanation_of_column1, "
    "i.e. what the column represents | sample_value_of_column1 |\n"
    "| name_of_column2 | type_of_column2 | explanation_of_column2, "
    "i.e. what the column represents | sample_value_of_column2 |\n"
    "| ... | ... | ... | ... |\n"
    "</output_format>"
).strip()
...
class TabularDataToolkit(AsyncBaseToolkit):
    def __init__(self, config: ToolkitConfig | None = None):
        ...
        self.llm = SimplifiedAsyncOpenAI(
            **(
                self.config.config_llm.model_provider.model_dump()
    ...
    def get_tabular_columns(
        self, file_path: str, return_feat: list[str] | None = None
    ) -> str:
        ...
        if not os.path.exists(file_path):
            return self._stringify_column_info(
        ...
        try:
            ...
            for col in df.columns:
                try:
                    ...
                    column_info.append(
                        {"column_name": str(col), "type": dtype, "sample": sample_str}
                    )
            ...
            return self._stringify_column_info(
                [{"error": error_msg}], return_feat=return_feat
            )
    ...
    def _stringify_column_info(
        self, column_info: list[dict], return_feat: list[str] | None = None
    ) -> str:
        ...
        for i, col in enumerate(column_info):
            col_str = json.dumps(
                {k: col[k] for k in return_keys if k in col}, ensure_ascii=False
            )
            lines.append(f"- Column {i + 1}: {col_str}")
        return "\n".join(lines)
