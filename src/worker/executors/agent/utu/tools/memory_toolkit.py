...
class SimpleMemoryToolkit(AsyncBaseToolkit):
    ...
    def _write_memory(self, content: str) -> str:
        ...
        if self.full_memory:
            ...
            return (
                "Warning: Overwriting existing content. Previous content was:\n"
                f"{previous}\n\n"
    ...
    def _edit_memory(self, old_string: str, new_string: str) -> str:
        ...
        if count > 1:
            return (
                ...
                "Please confirm which occurrence to replace or use more specific "
                "context."
            )
    ...
    async def simple_memory(
        ...
        """Tool for managing persistent text memory with read, write and edit
        operations.
        ...
        Args:
            action (Literal["read", "write", "edit"]: The action to perform on the
                memory.
            content (str, optional): The content to write to the memory. Defaults to "".
            old_string (str, optional): The string to replace in the memory.
                Defaults to "".
            new_string (str, optional): The string to replace the old string with in
                the memory. Defaults to "".
        """
        if action == "read":
            ...
            result = (
                ...
                "Valid actions are read, write, edit."
            )
        return result
...
class CompactifyMemoryToolkit(AsyncBaseToolkit):
    ...
    Applies the context manager's truncation strategy to compress the conversation
    history. This tool adapts to different context management approaches
    (summarization, simple truncation, etc.).
    """

    async def compactify_memory(
        self,
    ) -> str:
        """Compactifies the conversation memory using the configured context management
        strategy. Use this tool when the conversation is long and you need to free
        up context space. Helps maintain conversation continuity while staying within
        token limits.
        """
