...
from google.genai.types import HttpOptions, Part  # type: ignore[import-untyped]
...
class VideoToolkit(AsyncBaseToolkit):
    def __init__(self, config: ToolkitConfig | None = None) -> None:
        ...
        self.model = self.config.config.get("google_model", "gemini-1.5-video-preview")
    ...
    async def video_qa(self, video_url: str, question: str) -> str:
        ...
        response = self.client.models.generate_content(
            model=self.model,
            contents=[question, video_part],  # type: ignore
        )
