...
class BashRemoteToolkit(AsyncBaseToolkit):
    ...
    async def build(self):
        ...
        with httpx.Client() as client:
            ...
            response = client.post(
                f"{self.server_url}/api/create_terminal",
    ...
    async def cleanup(self):
        ...
        with httpx.Client() as client:
            ...
            }  # can automatically close the server with the session_id
            response = client.post(
                f"{self.server_url}/api/close_server", headers=self.headers, json=data
            )
        if not (response.status_code == 200):
            logger.info(
                f"Failed to close terminal with session_info: {self.session_info}"
            )
    ...
    async def exec(self, cmd: str) -> str:
        ...
        with httpx.Client(timeout=httpx.Timeout(60)) as client:
            ...
            response = client.post(
                f"{self.server_url}/api/execute", headers=self.headers, json=data
            )
        assert response.status_code == 200, response.text
        data = response.json()
        return str(
