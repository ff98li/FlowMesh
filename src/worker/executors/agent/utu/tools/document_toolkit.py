...
class DocumentToolkit(AsyncBaseToolkit):
    def __init__(self, config: ToolkitConfig | None = None) -> None:
        """Initialize the DocumentToolkit, with configured parser and llm."""
    ...
    def handle_path(self, path: str) -> str:
        ...
        if FileUtils.is_web_url(path):
            ...
            if not fn.exists():
                path = FileUtils.download_file(path, str(fn))
    ...
    async def document_qa(self, document_path: str, question: str | None = None) -> str:
        ...
        Args:
            ...
            question (str, optional): The question to answer. If not provided, return
        ...
        if question:
            messages.append(
                {
                    "role": "user",
                    "content": TOOL_PROMPTS["document_qa"].format(question=question),
                }
            )
        else:
            messages.append(
                {"role": "user", "content": TOOL_PROMPTS["document_summary"]}
            )
        output = await self.llm.query_one(
            messages=messages, **self.config.config_llm.model_params.model_dump()
        )
        if not question:
            output = (
                "You did not provide a particular question, so here is a "
                f"detailed caption for the document: {output}"
            )
        return output
