...
from .base import register_tool as register_tool
...
from .utils import get_tools_map as get_tools_map
from .utils import get_tools_schema as get_tools_schema
...
def get_toolkits_map(names: list[str] | None = None) -> dict[str, AsyncBaseToolkit]:
    ...
    if names is None:
        ...
        assert all(
            name in TOOLKIT_MAP for name in names
        ), f"Error config tools: {names}"
    for name in names:
