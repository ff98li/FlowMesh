...
class DataClassWithStreamEvents:
    ...
    _event_queue: asyncio.Queue[StreamEvent | AgentEvent | QueueCompleteSentinel] = (
        field(default_factory=asyncio.Queue, repr=False)
    )
    # Store the asyncio tasks that we're waiting on
    _run_impl_task: asyncio.Task[Any] | None = field(default=None, repr=False)

    async def stream_events(self) -> AsyncIterator[StreamEvent | AgentEvent]:
        while True:
            self._check_errors()
            ...
            # print(
            #     f"self._is_complete: {self._is_complete}, "
            #     f"self._event_queue: {self._event_queue}"
            # )
...
class TaskRecorder(DataClassWithStreamEvents):
    ...
    def add_run_result(self, run_result: RunResult, agent_name: str | None = None):
        ...
        self.trajectories.append(
            AgentsUtils.get_trajectory_from_agent_result(run_result, agent_name)
        )
