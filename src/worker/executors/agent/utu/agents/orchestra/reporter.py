...
class ReporterAgent:
    def __init__(self, config: AgentConfig):
        ...
        self.llm = SimplifiedAsyncOpenAI(
            **self.config.reporter_model.model_params.model_dump()
        )
    ...
    async def report(self, task_recorder: OrchestraTaskRecorder) -> AnalysisResult:
        ...
        query = self.template.render(
            question=task_recorder.task, trajectory=task_recorder.get_trajectory_str()
        )
        response = await self.llm.query_one(
            messages=query, **self.config.reporter_model.model_params.model_dump()
        )
