...
class BaseWorkerAgent:
    ...
    async def work(
        self, task_recorder: OrchestraTaskRecorder, subtask: Subtask
    ) -> WorkerResult:
...
class SimpleWorkerAgent(BaseWorkerAgent):
    ...
    async def work(
        ...
        return WorkerResult(
            ...
            trajectory=AgentsUtils.get_trajectory_from_agent_result(
                recorder.get_run_result()
            ),
        )
    ...
    def work_streamed(
        ...
        run_result_streaming = self.agent.run_streamed(
            aug_task, trace_id=task_recorder.trace_id
        )
        result = WorkerResult(
            task=subtask.task,
            output="",
