...
from ..common import AgentEvent, DataClassWithStreamEvents, TaskRecorder
...
class OrchestraTaskRecorder(TaskRecorder):
    plan: CreatePlanResult = field(default_factory=CreatePlanResult)
        ...
        return "\n".join(
            [
                ...
                for i, (r, t) in enumerate(
                    zip(self.task_records, self.plan.todo, strict=False), 1
                )
            ]
        )
...
@dataclass
class OrchestraStreamEvent(AgentEvent):
