...
class OutputParser:
    def __init__(self):
        ...
        # self.next_step_pattern = r"<next_step>\s*<agent>\s*(.*?)\s*</agent>\s*<task>\s*(.*?)\s*</task>\s*</next_step>"  # noqa: E501
    ...
    def _extract_plan(self, text: str) -> list[Subtask]:
        ...
        tasks = []
        task_pattern = r'\{"agent_name":\s*"([^"]+)",\s*"task":\s*"([^"]+)",\s*"completed":\s*(true|false)\s*\}'  # noqa: E501
        ...
        for agent_name, task_desc, completed_str in task_matches:
            ...
            tasks.append(
                Subtask(agent_name=agent_name, task=task_desc, completed=completed)
            )
        return tasks
...
class PlannerAgent:
    def __init__(self, config: AgentConfig):
        ...
        self.llm = SimplifiedAsyncOpenAI(
            **self.config.planner_model.model_provider.model_dump()
        )
    ...
    def _load_planner_examples(self) -> list[dict]:
        ...
        if examples_path and pathlib.Path(examples_path).exists():
            ...
            examples_path = (
                pathlib.Path(__file__).parent / "data" / "planner_examples.json"
            )
    ...
    async def create_plan(
        self, task_recorder: OrchestraTaskRecorder
    ) -> CreatePlanResult:
        ...
        response = await self.llm.query_one(
            messages=messages, **self.config.planner_model.model_params.model_dump()
        )
        return self.output_parser.parse(response)

    def _format_planner_examples(self, examples: list[dict]) -> str:
        # format examples to string. example:
        # {question, available_agents, analysis, plan}
        examples_str = []
