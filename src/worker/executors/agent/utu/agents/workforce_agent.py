...
class WorkforceAgent(BaseAgent):
    ...
    async def run(
        ...
        for name, config in self.config.workforce_executor_agents.items():
            executor_agent_group[name] = ExecutorAgent(
                config=config, workforce_config=self.config
            )

        recorder = WorkspaceTaskRecorder(
            overall_task=input,
            executor_agent_kwargs_list=self.config.workforce_executor_infos,
        )
        ...
        with trace(workflow_name=self.name, trace_id=trace_id):
            ...
            while recorder.has_uncompleted_tasks:
                ...
                logger.info(
                    f"Assign task: {next_task.task_id} assigned to "
                    f"{next_task.assigned_agent}"
                )

                # * 3. execute task
                logger.info(f"Executing task: {next_task.task_id}")
                assert next_task.assigned_agent is not None
                await executor_agent_group[next_task.assigned_agent].execute_task(
                    recorder=recorder, task=next_task
                )
                logger.info(f"Task {next_task.task_id} result: {next_task.task_result}")
                await planner_agent.plan_check(recorder, next_task)
                logger.info(
                    f"Task {next_task.task_id} checked: {next_task.task_status}"
                )

                # * 4. update plan
                if not recorder.has_uncompleted_tasks:  # early stop
                    break
                plan_update_choice = await planner_agent.plan_update(
                    recorder, next_task
                )
                logger.info(f"Plan update choice: {plan_update_choice}")
                if plan_update_choice == "stop":
                    logger.info(
                        "Planner determined overall task is complete, stopping "
                        "execution"
                    )
                    break
