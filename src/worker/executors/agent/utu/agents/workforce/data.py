...
class Subtask:
    ...
    task_result_detailed: str | None = None
...
class WorkspaceTaskRecorder(TaskRecorder):
    ...
    def executor_agents_names(self) -> str:
        return str(
            [agent_kwargs["name"] for agent_kwargs in self.executor_agent_kwargs_list]
        )
    ...
    def formatted_task_plan(self) -> str:
        ...
        for task in self.task_plan:
            formatted_plan_list.append(
                f"{task.task_id}. {task.task_name} - Status: {task.task_status}"
            )
    ...
    def plan_update(self, task: Subtask, updated_plan: list[str]) -> None:
        ...
        new_tasks = [
            Subtask(task_id=task.task_id + i, task_name=t)
    ...
    def get_next_task(self) -> Subtask:
        ...
        for task in self.task_plan:
            if task.task_status == "not started":
                return task
        raise ValueError("No uncompleted tasks.")
