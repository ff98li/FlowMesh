    ...
    async def execute_task(
        ...
        while tries <= self.max_tries:
            try:
                ...
                logger.info(
                    f"Task '{task.task_name}' reflection: {reflection_res.final_output}"
                )

                logger.warning(
                    f"Task '{task.task_name}' not completed. Retrying... "
                    f"(Attempt {tries}/{self.max_tries})"
                )
                tries += 1

            except Exception as e:
                logger.error(
                    f"Error executing task `{task.task_name}` on attempt "
                    f"{tries}: {str(e)}"
                )
                tries += 1
        ...
        if executor_res is None:
            logger.error(
                f"Task `{task.task_name}` execution failed after {tries} attempts!"
            )
            task.task_result = final_result or ""
            task.task_status = "failed"
            return

        recorder.add_run_result(
            executor_res.get_run_result(), "executor"
        )  # add executor trajectory
        task.task_result = final_result or ""
        task.task_status = "completed"

        if self.return_summary:
            # WARNING: reset instructions is dangerous! DO NOT use here!
            # self.executor_agent.set_instructions(
            #     PROMPTS["TASK_SUMMARY_SYSTEM_PROMPT"]
            # )
            ...
            recorder.add_run_result(
                summary_response.get_run_result(), "executor_summary"
            )  # add executor trajectory
            task.task_result_detailed, task.task_result = (
                summary_response.final_output,
                summary_response.final_output,
            )
            logger.info(
                "Task result summarized: "
                f"{task.task_result_detailed} -> {task.task_result}"
            )

    def _parse_task_check_result(self, response) -> bool:
        task_check_result = re.search(
            r"<task_check>(.*?)</task_check>", response, re.DOTALL
        )
