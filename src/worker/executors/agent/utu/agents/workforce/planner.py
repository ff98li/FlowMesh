...
class PlannerAgent:
    ...
    async def plan_task(self, recorder: WorkspaceTaskRecorder) -> None:
        ...
        recorder.add_run_result(
            plan_recorder.get_run_result(), "planner"
        )  # add planner trajectory

        # parse tasks
        pattern = "<task>(.*?)</task>"
        tasks_content: list[str] = re.findall(
            pattern, plan_recorder.final_output, re.DOTALL
        )
        tasks_content = [task.strip() for task in tasks_content if task.strip()]
        tasks = [
            Subtask(task_id=i + 1, task_name=task)
            for i, task in enumerate(tasks_content)
        ]
    ...
    async def plan_update(self, recorder: WorkspaceTaskRecorder, task: Subtask) -> str:
        ...
        previous_task_plan = "\n".join(
            f"{task}" for task in task_plan_list[: last_task_id + 1]
        )
        ...
        recorder.add_run_result(
            plan_update_recorder.get_run_result(), "planner"
        )  # add planner trajectory
        choice, updated_plan = self._parse_update_response(
            plan_update_recorder.final_output
        )
    ...
    def _parse_update_response(self, response: str) -> tuple[str, list[str] | None]:
        ...
        if match_choice:
            ...
            if choice not in ["continue", "update", "stop"]:
                logger.warning(
                    f"Unexpected choice value: {choice}. Defaulting to 'continue'."
                )
                choice = "continue"
        else:
        ...
        updated_tasks = None
        if choice == "update":
            pattern_updated_plan = (
                r"<updated_unfinished_task_plan>(.*?)</updated_unfinished_task_plan>"
            )
            ...
            if match_updated_plan:
                ...
                if not task_matches:
                    ...
                    task_matches = re.findall(
                        task_id_pattern, updated_plan_content, re.DOTALL
                    )

                updated_tasks = [task.strip() for task in task_matches if task.strip()]
                if not updated_tasks:
                    logger.warning(
                        "No tasks found in updated plan. Defaulting to None."
                    )
                    updated_tasks = None
            else:
    ...
    def _parse_check_response(self, response: str) -> str:
        ...
        if match:
            ...
            if task_status in ["success", "failed", "partial success"]:
                ...
                logger.warning(
                    f"Unexpected task status value: {task_status}. "
                    "Defaulting to 'partial success'."
                )
                return "partial success"
        else:
            logger.warning(
                "No task status found in response. Defaulting to 'partial success'."
            )
            return "partial success"
