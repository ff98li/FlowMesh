...
class AnswererAgent:
    """Answer extractor that handles final answer generation from task execution
    results."""
    ...
    async def extract_final_answer(self, recorder: WorkspaceTaskRecorder) -> str:
        ...
        recorder.add_run_result(
            final_recorder.get_run_result(), "answerer"
        )  # add answerer trajectory
    ...
    def _parse_final_response(self, response: str) -> str:
        ...
        if not answer_match:
            raise ValueError(f"Final answer not found in response:\n{response}")
        final_answer = answer_match.group(1).strip()
        return final_answer

    async def answer_check(
        self, question: str, model_answer: str, ground_truth: str
    ) -> bool:
        """Check if model answer and ground truth are semantically equivalent using
        LLM."""
