...
class SimpleAgent(BaseAgent):
    """A simple agent with env, tools, mcps, and context manager, wrapped on
    openai-agents."""
    ...
    def __init__(
        ...
        *,
        ...
        self.current_agent: Agent | None = None  # move to task recorder?
        self.input_items: list[TResponseInputItem] = []

        self._run_hooks: RunHooks | None = None
    ...
    def _get_model(
        self, config: AgentConfig, model: str | Model | None = None
    ) -> Model:
    ...
    def _get_model_settings(
        self, config: AgentConfig, model_settings: ModelSettings | None = None
    ) -> ModelSettings:
    ...
    async def build(self, trace_id: str | None = None):
        """Build the agent"""
        if self._initialized:
            logger.info("Agent already initialized! Skipping build.")
            return
        self.env = await get_env(
            self.config, trace_id or AgentsUtils.gen_trace_id()
        )  # Pass trace_id
        await self.env.build()
        self.current_agent = Agent(
            name=self.config.agent.name or "",
    ...
    async def _load_toolkit(self, toolkit_config: ToolkitConfig) -> AsyncBaseToolkit:
        if toolkit_config.name not in TOOLKIT_MAP:
            raise ValueError(f"Unknown builtin toolkit: {toolkit_config.name}")
        logger.info(
            f"Loading builtin toolkit `{toolkit_config.name}` with config "
            f"{toolkit_config}"
        )
    ...
    async def _load_customized_toolkit(
        self, toolkit_config: ToolkitConfig
    ) -> AsyncBaseToolkit:
        logger.info(
            f"Loading customized toolkit `{toolkit_config.name}` with config "
            f"{toolkit_config}"
        )
        assert (
            toolkit_config.customized_filepath is not None
            and toolkit_config.customized_classname is not None
        )
        toolkit_class = load_class_from_file(
            toolkit_config.customized_filepath, toolkit_config.customized_classname
        )
        toolkit = await self._tools_exit_stack.enter_async_context(
            toolkit_class(toolkit_config)
        )
        self._toolkits.append(toolkit)
        return toolkit
    ...
    async def _load_mcp_server(self, toolkit_config: ToolkitConfig) -> MCPServer:
        logger.info(
            f"Loading MCP server `{toolkit_config.name}` with params "
            f"{toolkit_config.config}"
        )
        match toolkit_config.mcp_transport:
            case "stdio":
                server = await self._mcps_exit_stack.enter_async_context(
                    MCPServerStdio(
                        ...
                        params=toolkit_config.config,  # type: ignore
            ...
            case _:
                raise ValueError(
                    f"Unknown MCP transport: {toolkit_config.mcp_transport}"
                )
    ...
    def _get_run_config(self) -> RunConfig:
        ...
        run_config = RunConfig(
            ...
            workflow_name=self.config.agent.name or "",
        )
        return run_config
    ...
    async def run(
        ...
        **_,
        ...
        task_recorder = TaskRecorder(input, trace_id=trace_id)
        ...
        if save:
            ...
            )  # NOTE: actually, there are only one agent in SimpleAgent
        return task_recorder

    def run_streamed(
        self, input: str | list[TResponseInputItem], trace_id: str | None = None
    ) -> RunResultStreaming:
