...
class OrchestraAgent(BaseAgent):
    ...
    def _setup_workers(self) -> dict[str, BaseWorkerAgent]:
        ...
        for name, config in self.config.workers.items():
            assert (
                config.type == "simple"
            ), f"Only support SimpleAgent as worker in orchestra agent, get {config}"
    ...
    async def run(
        self, input: str, trace_id: str | None = None, **_
    ) -> OrchestraTaskRecorder:
    ...
    def run_streamed(
        self, input: str, trace_id: str | None = None
    ) -> OrchestraTaskRecorder:
        trace_id = trace_id or AgentsUtils.gen_trace_id()
        logger.info(f"> trace_id: {trace_id}")
        ...
        with trace(workflow_name="orchestra_agent", trace_id=trace_id):
            task_recorder = OrchestraTaskRecorder(task=input, trace_id=trace_id)
            # Kick off the actual agent loop in the background and return the streamed
            # result object.
            task_recorder._run_impl_task = asyncio.create_task(
                self._start_streaming(task_recorder)
            )
        return task_recorder

    async def _start_streaming(self, task_recorder: OrchestraTaskRecorder):
        task_recorder._event_queue.put_nowait(
            AgentUpdatedStreamEvent(new_agent=self.planner_agent)  # type: ignore
        )
        plan = await self.plan(task_recorder)
        task_recorder._event_queue.put_nowait(
            OrchestraStreamEvent(name="plan", item=plan)
        )
        for task in task_recorder.plan.todo:
            # print(f"> processing {task}")
            # DO NOT send this event because Runner will send it
            # task_recorder._event_queue.put_nowait(
            ...
            stream = result_streaming.stream
            assert stream is not None
            async for event in stream.stream_events():
                task_recorder._event_queue.put_nowait(event)
            result_streaming.output = stream.final_output
            result_streaming.trajectory = AgentsUtils.get_trajectory_from_agent_result(
                stream
            )
            task_recorder.add_worker_result(result_streaming)
            # print(f"< processed {task}")
        task_recorder._event_queue.put_nowait(
            AgentUpdatedStreamEvent(new_agent=self.reporter_agent)  # type: ignore[arg-type]
        )
        result = await self.report(task_recorder)
        task_recorder.set_final_output(result.output)
        task_recorder._event_queue.put_nowait(
            OrchestraStreamEvent(name="report", item=result)
        )
    ...
    async def plan(self, task_recorder: OrchestraTaskRecorder) -> CreatePlanResult:
        ...
        with function_span("planner") as span_planner:
            ...
            assert all(
                t.agent_name in self.worker_agents for t in plan.todo
            ), f"agent_name in plan.todo must be in worker_agents, get {plan.todo}"
            task_recorder.set_plan(plan)
            span_planner.span_data.input = json.dumps(
                {"input": task_recorder.task}, ensure_ascii=False
            )
            span_planner.span_data.output = plan.to_dict()
        return plan

    async def work(
        self, task_recorder: OrchestraTaskRecorder, task: Subtask
    ) -> WorkerResult:
        """Step2: Work"""
        worker_agent = self.worker_agents[task.agent_name]
            ...
            span_fn.span_data.input = json.dumps(
                {
                    ...
                    "task_records": [
                        {"task": r.task, "output": r.output}
                        for r in task_recorder.task_records
                    ],
                },
                ensure_ascii=False,
            )
