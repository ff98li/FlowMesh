...
from ..agents import AgentEvent, SimpleAgent
...
class SimpleAgentGeneratedEvent(BaseModel, AgentEvent):
...
class SimpleAgentGenerator:
    ...
    async def build(self):
        ...
        self.agent_1 = SimpleAgent(
            ...
            instructions=self.jinja_env.get_template(
                "requirements_clarification.j2"
            ).render(),
    ...
    def run_streamed(self, user_input: str) -> GeneratorTaskRecorder:
        with trace("simple_agent_generator"):
            ...
            task_recorder._run_impl_task = asyncio.create_task(
                self._start_streaming(task_recorder, user_input)
            )
        return task_recorder

    async def _start_streaming(
        self, task_recorder: GeneratorTaskRecorder, user_input: str
    ):
        await self.build()
        await self.step1(task_recorder, user_input)
        await self.step2(task_recorder)
    ...
    def format_config(self, task_recorder: GeneratorTaskRecorder) -> tuple[str, str]:
        toolkits_includes = []
        toolkits_configs = []
        if task_recorder.selected_tools is not None:
            for toolkit_name, tool_names in task_recorder.selected_tools.items():
                toolkits_includes.append(
                    f"- /tools/{toolkit_name}@toolkits.{toolkit_name}"
                )
                toolkits_configs.append(
                    f"{toolkit_name}: {json.dumps({'activated_tools': tool_names})}"
                )
        assert task_recorder.instructions is not None
    ...
    async def step2(self, task_recorder: GeneratorTaskRecorder) -> None:
        ...
        tools_descs: list[str] = []
        tool_to_toolkit_name: dict[str, str] = {}
        for toolkit_name in available_toolkits:
            ...
            tools_descs.extend(
                f"- {tool.name}: {tool.description}" for tool in tools_schema.values()
            )
            tool_to_toolkit_name.update(
                {tool.name: toolkit_name for tool in tools_schema.values()}
            )
