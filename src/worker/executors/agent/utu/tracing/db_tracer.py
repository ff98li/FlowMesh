...
class DBTracingProcessor(TracingProcessor):
    ...
    def __init__(self) -> None:
        if not SQLModelUtils.check_db_available():
            logger.warning(
                "DB_URL not set or database connection failed! Tracing will not be "
    ...
    def on_span_end(self, span: Span[Any]) -> None:
        ...
        if isinstance(data, GenerationSpanData):
            ...
            with SQLModelUtils.create_session() as session:
                assert not isinstance(data.input, str)
                ...
                session.add(
                    GenerationTracingModel(
                        ...
                        model=OpenAIUtils.maybe_basemodel_to_dict(  # type: ignore
                            data.response.model
                        ),
