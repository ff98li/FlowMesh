...
def setup_otel_tracing(
    ...
    if not endpoint or not project_name:
        logger.warning(
            "PHOENIX_ENDPOINT or PHOENIX_PROJECT_NAME is not set! Skipping "
    ...
    logger.info(
        f"Setting up OpenTelemetry tracing with endpoint: {endpoint}, project name: "
        f"{project_name}"
    )
    OTEL_TRACING_PROVIDER = TracerProvider(
        resource=Resource({ResourceAttributes.PROJECT_NAME: project_name})
    )
    OTEL_TRACING_PROVIDER.add_span_processor(
    ...
    # use `set_trace_processors` instead of `add_trace_processor` to remove default
    # processors
    OpenAIAgentsInstrumentor().instrument(
        tracer_provider=OTEL_TRACING_PROVIDER, exclusive_processor=True
    )
...
def setup_db_tracing() -> None:
    ...
    if not SQLModelUtils.check_db_available():
        logger.warning(
            "DB_URL not set or database connection failed! Tracing will not be stored "
            "into database!"
        )
        return
