...
class PhoenixUtils:
    ...
    def get_trace_url_by_id(self, trace_id: str) -> str | None:
        # get trace by trace_id in @openai-agents, see the trick in
        # OpenInferenceTracingProcessor
        spans_df = self.get_spans(
            condition=f"metadata['trace_id'] == '{trace_id}'",
            select=["context.trace_id"],
            limit=1,
        )
        if spans_df.empty:
            return None
